...
latt_params_Ang = latt_params_arr*.5291772106721

# Read out exact lattice parameters
latts = np.array([3.12, 3.13, 3.14, 3.15, 3.16, 3.17, 3.18, 3.19, 3.20, 3.21, 3.22, 3.23, 3.24])
...
with open('./Data_A.txt', 'w') as file:
    ...
    for i in range(len(latt_params_arr)):
        file.write(f'{latts[i]}\t{energies_eV[i]}\n')

# 4) Plot Total Energy vs Lattice Parameters: Save as png
plt.figure()
plt.plot(latts, energies_eV, "o-", linewidth=1.5)
